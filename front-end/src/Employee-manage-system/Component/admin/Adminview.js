...
const Adminview = () => {
    ...
    const handlestop = async () => {
        try {
            const res = await axios.post(
                "http://localhost:4000/employee/taskstatus",
                chng
            );
            if (res.data) {
                alert("Success");
    ...
    const handledelete = async (id) => {
        try {
            const res = await axios.post(
                `http://localhost:4000/employee/taskdelete/${id}`
            );
            if (res.data) {
                setTaskemp("");
            }
    ...
    useEffect(() => {
        ...
        setTaskdata(
            taskfield.filter(
                (each) =>
                    each.employeeid === taskemp._id && each.status !== "stop"
            )
        );
    }, [taskemp]);
    ...
    const taskfetch = async () => {
        const res = await axios.get(
            "http://localhost:4000/employee/taskdetail"
        );
        if (res.data) {
    ...
    return (
        <div className="main  my-md-2 mx-4 mx-md-5 col-10 col-xl-12 ">
            ...
            {taskfield.filter((each) => each.employeeid === taskemp._id)
                .length === 0 ? (
