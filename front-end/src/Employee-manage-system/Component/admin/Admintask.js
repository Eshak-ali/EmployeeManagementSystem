    ...
    if (taskfield?.name !== "" && taskfield?.description !== "") {
      try {
        const res = await axios.post(
          `${process.env.REACT_APP_API}/employee/task/${employeeid}`,
          taskfield
        );
        if (res.data) {
          alert("success");
  ...
  return (
    <div className="col-12 p-2">
      <div className=" d-flex align-item-center justify-content-around mt-2">
        <p id="as">
          <Link to="/task">
            <button className="btn  text-dark" onClick={() => setbtn("assign")}>
              Task_Assign
            </button>
          </Link>
        </p>
        <p id="ve">
          <Link to="assign">
            <button className="btn  text-dark" onClick={() => setbtn("view")}>
              Task_View
            </button>
          </Link>
        </p>
